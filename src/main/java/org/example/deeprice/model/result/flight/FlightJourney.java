            ...
            for (Flight flight : flights.getFlights()) {
                ...
                for (int i = 0; i < flight.getRatings().length; i++) {
                    if (i == 0 && flight.getRatings()[i] > 0) {
                        sb.append("<div id=\"food-req\">" + flight.getRatings()[i] +"<i class=\"fas fa-utensils\"></i>" + AirlinesMap.IATA_AIRLINES_MAP.get(flight.getCarrierCode()) + " is known for it's delicious meals.</div>");
                    }
                    if (i == 1 && flight.getRatings()[i] > 0) {
                        sb.append("<div id=\"customer-service-req\"> " + flight.getRatings()[i] + " <i class=\"fas fa-headset\"></i> " + AirlinesMap.IATA_AIRLINES_MAP.get(flight.getCarrierCode()) + " is well-known for it's good customer service.</div>");
                    }
                    if (i == 2 && flight.getRatings()[i] > 0) {
                        sb.append("<div id=\"seat-req\">" + flight.getRatings()[i] + "<i class=\"fas fa-chair\"></i> " + AirlinesMap.IATA_AIRLINES_MAP.get(flight.getCarrierCode()) + " is well-known for it's comfortable chairs.</div>");
                    }
                    if (i == 3 && flight.getRatings()[i] > 0) {
                        sb.append("<div id=\"legroom-req\">" + flight.getRatings()[i] +"<i class=\"fas fa-shoe-prints\"></i> " + AirlinesMap.IATA_AIRLINES_MAP.get(flight.getCarrierCode()) + " is well-known for it's generous legroom.</div>");
                    }
                }
                ...
                for (int i = 0; i < flight.getRatings().length; i++) {
                    if (i == 0 && flight.getRatings()[i] < 0) {
                        sb.append("<div id=\"food-req\">" + flight.getRatings()[i] +"<i class=\"fas fa-utensils\"></i>" + AirlinesMap.IATA_AIRLINES_MAP.get(flight.getCarrierCode()) + " does maybe not meet your requirements, as the the food there is not highly viewed by customers.</div>");
                    }
                    if (i == 1 && flight.getRatings()[i] < 0) {
                        sb.append("<div id=\"customer-service-req\">" + flight.getRatings()[i] + "<i class=\"fas fa-headset\"></i> " + AirlinesMap.IATA_AIRLINES_MAP.get(flight.getCarrierCode()) + " is <span style=\"font-weight: bold;\">not</span> well-known for it's good customer service.</div>");
                    }
                    if (i == 2 && flight.getRatings()[i] < 0) {
                        sb.append("<div id=\"seat-req\">" + flight.getRatings()[i] +"<i class=\"fas fa-chair\"></i> " + AirlinesMap.IATA_AIRLINES_MAP.get(flight.getCarrierCode()) + " is <span style=\"font-weight: bold;\">not</span> well-known for it's comfortable chairs.</div>");
                    }
                    if (i == 3 && flight.getRatings()[i] < 0) {
                        sb.append("<div id=\"legroom-req\">" + flight.getRatings()[i] + "<i class=\"fas fa-shoe-prints\"></i> " + AirlinesMap.IATA_AIRLINES_MAP.get(flight.getCarrierCode()) + " is <span style=\"font-weight: bold;\">not</span> well-known for it's legroom.</div>");
                    }
                }
                sb.append("<br>");
            }
